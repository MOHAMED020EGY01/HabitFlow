...
interface HabitDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertCycleHistory(history: com.example.core.model.entity.HabitCycleHistoryEntity): Long

    @Query("SELECT * FROM habit_cycle_history WHERE habitId = :habitId ORDER BY cycleEndDate DESC")
    fun getCycleHistoryForHabit(habitId: Int): Flow<List<com.example.core.model.entity.HabitCycleHistoryEntity>>

    @Query("SELECT * FROM habit_cycle_history WHERE habitId = :habitId ORDER BY cycleEndDate DESC")
    suspend fun getCycleHistoryForHabitSync(habitId: Int): List<com.example.core.model.entity.HabitCycleHistoryEntity>
