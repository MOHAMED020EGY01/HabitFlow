...
object AppFormatters {
    ...
    fun formatTime(hour: Int, minute: Int, langCode: String? = null): String {
        ...
        val localizedCtx = com.example.util.LocaleDirectionHelper.getLocalizedContext(app, effectiveLang)
        val amPm = if (hour < 12) {
    ...
    fun formatDate(date: LocalDate, pattern: String, langCode: String? = null): String {
        val app = com.example.HabitApplication.instance
        val effectiveLang = langCode ?: app.currentLanguageCode
        val locale = com.example.util.LocaleDirectionHelper.getLocale(effectiveLang)
