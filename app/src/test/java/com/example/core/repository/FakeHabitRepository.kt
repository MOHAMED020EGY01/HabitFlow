...
import com.example.core.model.domain.HabitWithWidgetInfo
...
class FakeHabitRepository : HabitRepository {
    ...
    override suspend fun setHabitActive(habitId: Int, isActive: Boolean, startedAt: Long?, status: com.example.core.model.domain.HabitStatus, inactiveSince: Long?) {
        ...
        if (habit != null) {
            habitsMap[habitId] = habit.copy(isActive = isActive, startedAt = startedAt, status = status)
            updateFlows()
        }
    }

    override suspend fun getRecentInactiveHabits(limit: Int): List<Habit> {
        return habitsMap.values.filter { !it.isActive }.take(limit)
    }
    ...
    override fun getAllNotificationsFlow(): Flow<List<com.example.core.model.domain.HabitNotification>> {
        return MutableStateFlow(emptyList())
    }

    override suspend fun insertNotification(notification: com.example.core.model.domain.HabitNotification) {}
