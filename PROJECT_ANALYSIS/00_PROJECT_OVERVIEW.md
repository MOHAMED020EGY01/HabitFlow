...
* **العمل دون اتصال بالإنترنت (Local-First)**: يتم تخزين كافة البيانات محلياً على قاعدة بيانات SQLite باستخدام مكتبة Room دون الاعتماد على خوادم سحابية.
...
* **التذكيرات فائقة الموثوقية (Reliable Alarms)**: الاعتماد على نظام ذكي يدمج بين عمال WorkManager والخدمات الأمامية والمنبهات لضمان عدم تفويت التنبيهات.
...
* **Local-First Architecture**: All user data resides in a local SQLite database using Room, requiring zero internet dependency.
...
تتحكم فئة `HabitApplication` بدورة حياة التهيئة. يتم قراءة تفضيلات المستخدم للغة فوراً بشكل متزامن `runBlocking` لتطبيق اتجاه النصوص RTL أو Ltr لتجنب التحديثات المتقطعة للغة، بينما يتم تهيئة قاعدة البيانات Room ومستودع البيانات UseCases بشكل غير متزامن `applicationScope.async` خلف الستار للحفاظ على سرعة الاستجابة.

`HabitApplication` controls the startup lifecycle. User language options are fetched synchronously using `runBlocking` to enforce correct layout direction (RTL/LTR) instantly on draw, while database, repositories, and use case layers initialize asynchronously inside an `applicationScope.async` block.
...
* **Evidence / الأدلة**:
  ...
  - ملف البناء `app/build.gradle.kts` الذي يوثق إصدارات التشغيل ومستويات SDK.
